...
class TrafficLightsSensor(PseudoActor):
    ...
    def __init__(self, uid, name, parent, node, actor_list, world_actor_list):
        """
        Constructor
        ...
        self.world_actor_list = world_actor_list
    ...
    def control_command_updated(self, ros_traffic_light_control):
        # i = 0
        for ros_traffic_light_control,actor in enumerate(self.world_actor_list):
            # print(i, type(actor[0]), type(actor[1]))
            # if isinstance(actor, carla.libcarla.TrafficLight):
            #     print('111')
            # if i < 3:
            traffic_light=[]
            traffic_light.sort(key= lambda actor:actor.get_transform().rotation.yaw)
            print(ros_traffic_light_control)
            print(actor)
            if ros_traffic_light_control==0.0:
                actor.set_state(carla.TrafficLightState.Red)
        #     if ros_traffic_light_control == 1.0:
        #         print("green")
        #         actor[3].set_state(carla.TrafficLightState.Green)
        #         actor[1].set_green_time(99.0)
        #     elif ros_traffic_light_control == 2.0:
        #         print("red")
        #         actor[3].set_state(carla.TrafficLightState.Green)
        #     elif ros_traffic_light_control == 3.0:
        #         print("yellow")
        #         actor[3].set_state(carla.TrafficLightState.Green)
        #         actor[3].set_red_time(99.0)
        # carla.Client('localhost', 2000).get_world().freeze_all_traffic_lights(True)
            # i += 1